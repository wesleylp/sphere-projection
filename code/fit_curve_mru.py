...
# fit model
coef = np.polyfit(df["timestamp"], df["x"], 1)  # first-order fit (linear)
x_o, v = coef[1], coef[0]
x_pred = np.polyval(coef, df["timestamp"])
print(f"x0: {x_o:.4f} px")
print(f"v: {v:.4f} px/ms")
...
# plot model
t_model_plot = np.arange(df["timestamp"].min() - 1, df["timestamp"].max() + 0.5, 0.1)
x_model_plot = np.polyval(coef, t_model_plot)
plt.plot(
    t_model_plot,
    x_model_plot,
    color="black",
    label=f"Model: $x(t) = {x_o:.2f} {v:.2f}t$\n$R^2 = {R2:.4f}$",
)
...
plt.savefig(
    os.path.join(os.path.dirname(this_filepath), "mru_model.png"),
    bbox_inches="tight",
    dpi=600,
)

plt.savefig(
    os.path.join(os.path.dirname(this_filepath), "mru_model.pdf"),
    bbox_inches="tight",
    dpi=600,
)
